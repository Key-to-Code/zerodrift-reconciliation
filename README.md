...
**Live demo:** [zerodrift-reconciliation-engine.streamlit.app](https://zerodrift-reconciliation-engine.streamlit.app/) 

#### The backend runs on a free Render instance, so the first request after a period of inactivity can take 20-50s to wake up before it responds. That's cold-start latency, not a bug.
...
## **Where** and **why**  this project uses an LLM

**Where** The LLM appears in exactly one place: `src/agent/graph.py`, a bounded LangGraph diagnostic loop invoked from the orchestrator (`src/orchestration/batch_runner.py`) — and only for the subset of records that Layer 2's deterministic fast path (`src/matching/fast_path.py`) could not confidently resolve. Everything else in the system — the 3-hop matching cascade, the double-entry ledger, the largest-remainder UTR allocation, the cash forecaster — is plain deterministic code with zero model involvement. On the frozen 100-record benchmark, that means roughly 63 records never touch the model at all; the agent is only ever asked about the remaining ~37.

**Why** an LLM at all, rather than more rules. The 63% the fast path resolves are exact-key joins, regex/fuzzy narration matching, and amount/date-window checks — genuinely mechanical, and a model would only add cost, latency, and hallucination risk for no benefit there. The remaining records are different in kind, not just harder: telling AMEX_SURCHARGE apart from INTL_MARKUP means reading a merchant contract clause and weighing it against the settlement's own is_international flag; a missing_tax_line anomaly means recomputing what GST/TDS should have been and diagnosing which specific line was dropped; a cutoff_drift case means reasoning about a rail's settlement window against a business-day calendar. These are judgment calls over ambiguous evidence, not lookups — the kind of long tail that a rules engine would need constant, ever-multiplying special-casing to cover, and would still snap on the next anomaly shape nobody anticipated. An LLM with the right tools and constraints generalizes to a new case with no new code; a hardcoded rule table doesn't.

### Why it's bounded this tightly, not a general-purpose agent ? 
Giving a model unrestricted reach into a financial ledger is the actual risk this design exists to eliminate, so the agent is deliberately constrained on every axis:
- Read-only tools, capped at 3 calls per record (`src/agent/tools.py`) — it can query a merchant's contract, the current tax rules, and the settlement timing rules, and nothing else.
- No ledger write access, ever. The agent proposes a structured AgentResolution (`root_cause_code` + `quantified_delta_paise`, not prose); a separate gatekeeper (`gatekeeper_check`) decides whether to post it — and checks that decision against the graph's own real tool-call history, never the model's self-reported evidence.
- A forced honest exit. If the agent can't back a diagnosis with real evidence, it doesn't guess — it routes to honest_exception, posted to a suspense account rather than a wrong number.
- A gradeable output, not a vibe. Because the resolution schemcan diff `root_cause_code` and `quantified_delta_paise againstground_truth.json` — "the agent resolved it" becomes a checkable claim instead of an assertion.
- Every invocation logged and replayable (`data/agent_runs/`), sluate.py --replay with zero live calls if the network or theAPI is down.

***The result is that the LLM is used exactly where deterministic logic genuinely runs out, and nowhere else — and even there, it's a constrained diagnostic step with a human-auditable trail, not an autonomous actor with the keys to the books.***
